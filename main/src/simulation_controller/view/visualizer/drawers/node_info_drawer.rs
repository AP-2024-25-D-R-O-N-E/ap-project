...
use egui::{text::LayoutJob, CollapsingHeader, Color32, Context, Label, Layout, RichText, ScrollArea, TextEdit, TextFormat, Ui, Window};
    ...
    ui.horizontal(|ui| {
        ...
        let selected_text =
            match get_client_node_state(&mut state.node_info_section, node_index).current_peer {
                Some(id) => {
                    get_client_node_state(&mut state.node_info_section, node_index).unread_messages.remove(&id);
                    format!("Client {id}")},
        ...
        let mut string_job = LayoutJob::default();
            ...
            if !get_client_node_state(&mut state.node_info_section, node_index).unread_messages.is_empty() {
                string_job.append("   📲", 0.0, TextFormat {
            ...
            .show_ui(ui, |ui| {
                    ...
                    .clone()
                {
                    ...
                    let mut client_job = LayoutJob::default();
                        ...
                        if get_client_node_state(&mut state.node_info_section, node_index).unread_messages.contains(&node_id) {
                            client_job.append("   •", 0.0, TextFormat {
                                color: Color32::LIGHT_RED,
                                ..TextFormat::default()
                            });
                        }
